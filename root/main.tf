...
module "cloudfront" {
  source               = "../modules/cloudfront"
  alb_domain_name      = module.alb.alb_dns_name
  additional_domain_name = var.additional_domain_name  # This can be left as an empty string if not using any custom domain
  project_name         = module.vpc.project_name
    # Leave empty since you're not using a custom certificate/domain
}
